...
public class TaskLogBo extends BaseBo {
	...
	public TaskLogBo setJobId(String jobId) {
		return (TaskLogBo) setAttribute(ATTR_JOB_ID, jobId != null ? jobId.trim().toLowerCase() : null);
	}
	...
	public TaskLogBo setOutput(byte[] output) {
		return (TaskLogBo) setAttribute(ATTR_OUTPUT, output != null ? output : ArrayUtils.EMPTY_BYTE_ARRAY);
	}
	...
	public TaskLogBo setTimestampCreate(Date timestamp) {
		return (TaskLogBo) setAttribute(ATTR_TIMESTAMP_CREATE, timestamp != null ? timestamp : new Date());
	}
	...
	public TaskLogBo setTimestampPickup(Date timestamp) {
		...
		long duration = timestampCreate != null ? timestamp.getTime() - timestampCreate.getTime() : 0;
